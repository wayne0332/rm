    ...
    $.widget("metro.slider", {
        ...
        _create: function () {
            ...
            if (element.data('name') != undefined) {
                o.name = element.data('name');
            }
            if (element.data('canMove') != undefined) {
                o.canMove = element.data('canMove');
            }
        ...
        _placeMarker: function (value) {
            var size, size2, o = this.options, colorParts = 0, colorIndex = 0, colorDelta = 0,
                ...
                name = this.element.children('.slideName'),
            ...
            if (this.options._slider.vertical) {
                ...
                if (o.showHint) {
                    hint.html(Math.round(value) + "/" + o.max).css('top', size2 - hint.height() / 2);
                }
            } else {
                ...
                if (o.showHint) {
                    this._animate(hint.html(Math.round(value) + "/" + o.max).css('left', oldPos - hint.width() / 2), {left: size - hint.width() / 2});
                }
            }
            if(o.name != '') {
                name.attr("value", Math.round(value));
            }
        },
        ...
        _createSlider: function () {
            ...
            if (options.name != '') {
                name = $("<input/>").addClass("slideName").attr("name", options.name).attr("type", "hidden").appendTo(element);
            }
